...
void start()
{
	...
	std::cout << "5. Limbo (numbers between 1 and 10000)" << std::endl;
	std::cout << "6. Hell (numbers between 1 and 1000000)" << std::endl;
...
{
	if(level > 6 || level < 1){
		std::cout << "Error select between 1 through 6" << std::endl;
		int input = getInput();
		game(input);
	}
	else{
